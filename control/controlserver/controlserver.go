...
func (s *ControlServer) generateTailScaleIp() netaddr.IPPrefix {
	...
	ip, err := netaddr.ParseIPPrefix("100.112.127.84/32")
	if err != nil {
...
func (s *ControlServer) PollNetMapHandler(w http.ResponseWriter, r *http.Request) {
	...
	if nodeId, ok := s.knownHost[tailcfg.MachineKey(clientMachineKey)]; ok {
		...
		node := tailcfg.Node{
			...
			Addresses:         []netaddr.IPPrefix{s.generateTailScaleIp()}, // tailscale Ip generated by server
			AllowedIPs:        pollRequest.Hostinfo.RoutableIPs, // wireguard allowedIps
